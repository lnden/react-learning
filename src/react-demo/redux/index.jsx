    ...
    render() {
        return (
            <section>
                ...
                <h4>纯函数 概念</h4>
                <p>一个函数的返回结果只依赖于他的参数，并且在执行过程里面没有副作用，我们就把这个函数叫做纯函数。</p>
                <p>1).函数外部有一个变量，在函数内部调用，外部修改变量会影响函数内部的值，这就不属于以纯函数。</p>
                <p>2).函数外部有一个对象，在函数内部调用，函数内部修改对象的值，会对外部的对象产生影响，他就产生了副作用(可以把外部的变量放在内部使用)</p>
                <p>这里又涉及到深拷贝以及浅拷贝的问题！对象获得的只是一个内存地址，而不是真正拷贝，因此存在父对象被篡改的可能。</p>
            </section>
        )
    }
}
